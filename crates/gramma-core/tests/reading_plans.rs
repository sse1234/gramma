...
fn plans_import_replace_and_list() {
    ...
    let names: Vec<String> = library
        ...
        .map(|p| p.name)
        .collect();
    assert_eq!(names, ["Advent", "Partner"]);
}
