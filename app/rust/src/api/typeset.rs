...
pub fn init_typesetting(font_data: Vec<u8>) -> anyhow::Result<()> {
    FontMeasure::new(&font_data).ok_or_else(|| anyhow!("cannot parse font"))?;
    let _ = FONT_DATA.set(font_data);
    Ok(())
}
...
pub fn layout_chapter(
    ...
    let measure = FontMeasure::new(font).ok_or_else(|| anyhow!("cannot parse font"))?;
