...
void main() {
  ...
  final lines = [
    const LineView(
      imageLines: 0,
      runs: [],
    ), // leading spacer would never happen; edge case
    LineView(imageLines: 0, runs: [_run(1)]),
    const LineView(
      imageLines: 0,
      runs: [],
    ), // spacer before the section heading
    LineView(
      imageLines: 0,
      runs: [_run(2)],
    ), // heading line, anchored to verse 2
    LineView(imageLines: 0, runs: [_run(2), _run(3)]),
  ];

  test('start lookup walks forward past spacer lines', () {
    expect(
      verseAtLineStart(lines, 2),
      2,
  ...
  test('end lookup walks backward past spacer lines', () {
    expect(
      verseAtLineEnd(lines, 2),
      1,
      reason:
          'a spacer defers to the previous carrying line — never the '
          'whole chapter',
    );
    expect(verseAtLineEnd(lines, 4), 3);
    expect(
      verseAtLineEnd(lines, 0),
      null,
